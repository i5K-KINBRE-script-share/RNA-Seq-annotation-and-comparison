...
###############################################################################
print "###########################################################\n";
print "#  RNA-SeqAlign.pl Version 1.0                            #\n";
print "#                                                         #\n";
...
##############                get arguments                  ##################
###############################################################################
my ($r_list,$project_name,$transcriptome,$min_len,$labels,$sams);
my $convert_header = 0;
my $mapq = 10;
my $man = 0;
my $help = 0;
...
2) Index the de novo transcriptome.
 
 bash ~/test_de_novo_DE/cell_lines_qsubs/cell_lines_qsubs_index.sh
 
3) When these jobs and the idexing job is complete view the ".gd" files and then go to next step. Test completion by typing "status" in a Beocat session. Download the ".gd" files in the Project_name_prinseq directory and upload them to http://edwards.sdsu.edu/cgi-bin/prinseq/prinseq.cgi?report=1 to evaluate read quality pre and post cleaning
